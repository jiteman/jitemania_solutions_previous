...
CATCH_TEST_CASE("Stringifying std::chrono::duration helpers", "[toString][chrono]") {
    ...
    CATCH_REQUIRE(minute == seconds);
    CATCH_REQUIRE(hour != seconds);
    CATCH_REQUIRE(micro != milli);
    CATCH_REQUIRE(nano != micro);
}

CATCH_TEST_CASE("Stringifying std::chrono::duration with weird ratios", "[toString][chrono]") {
    ...
    CATCH_REQUIRE(half_minute != femto_second);
    CATCH_REQUIRE(pico_second != atto_second);
}

CATCH_TEST_CASE("Stringifying std::chrono::time_point<system_clock>", "[toString][chrono]") {
    auto now = std::chrono::system_clock::now();
    auto later = now + std::chrono::minutes(2);
    CATCH_REQUIRE(now != later);
}

CATCH_TEST_CASE("Stringifying std::chrono::time_point<Clock>", "[toString][chrono][!nonportable]") {
