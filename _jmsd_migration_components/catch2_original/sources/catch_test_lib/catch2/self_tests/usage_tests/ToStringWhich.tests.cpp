...
// Call the operator
CATCH_TEST_CASE( "stringify( has_operator )", "[toString]" ) {
    has_operator item;
    CATCH_REQUIRE( ::Catch::Detail::stringify( item ) == "operator<<( has_operator )" );
}

// Call the stringmaker
CATCH_TEST_CASE( "stringify( has_maker )", "[toString]" ) {
    has_maker item;
    CATCH_REQUIRE( ::Catch::Detail::stringify( item ) == "StringMaker<has_maker>" );
}

// Call the stringmaker
CATCH_TEST_CASE( "stringify( has_maker_and_operator )", "[toString]" ) {
    has_maker_and_operator item;
    CATCH_REQUIRE( ::Catch::Detail::stringify( item ) == "StringMaker<has_maker_and_operator>" );
}

CATCH_TEST_CASE("stringify( has_neither )", "[toString]") {
    has_neither item;
    CATCH_REQUIRE( ::Catch::Detail::stringify(item) == "{ !!! }" );
}

// Call the templated operator
CATCH_TEST_CASE( "stringify( has_template_operator )", "[toString]" ) {
    has_template_operator item;
    CATCH_REQUIRE( ::Catch::Detail::stringify( item ) == "operator<<( has_template_operator )" );
}


// Vectors...

CATCH_TEST_CASE( "stringify( vectors<has_operator> )", "[toString]" ) {
    std::vector<has_operator> v(1);
    CATCH_REQUIRE( ::Catch::Detail::stringify( v ) == "{ operator<<( has_operator ) }" );
}

CATCH_TEST_CASE( "stringify( vectors<has_maker> )", "[toString]" ) {
    std::vector<has_maker> v(1);
    CATCH_REQUIRE( ::Catch::Detail::stringify( v ) == "{ StringMaker<has_maker> }" );
}

CATCH_TEST_CASE( "stringify( vectors<has_maker_and_operator> )", "[toString]" ) {
    std::vector<has_maker_and_operator> v(1);
    CATCH_REQUIRE( ::Catch::Detail::stringify( v ) == "{ StringMaker<has_maker_and_operator> }" );
}
...
CATCH_TEST_CASE("stringify ranges", "[toString]") {
    CATCH_REQUIRE(::Catch::Detail::stringify(streamable_range{}) == "op<<(streamable_range)");
    CATCH_REQUIRE(::Catch::Detail::stringify(stringmaker_range{}) == "stringmaker(streamable_range)");
    CATCH_REQUIRE(::Catch::Detail::stringify(just_range{}) == "{ 1, 2, 3, 4 }");
    CATCH_REQUIRE(::Catch::Detail::stringify(disabled_range{}) == "{ !!! }");
}
