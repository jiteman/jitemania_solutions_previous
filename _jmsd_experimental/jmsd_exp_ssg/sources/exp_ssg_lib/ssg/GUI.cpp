...
void DrawInformation()
{
	...
	DrawMyText_int_box( WIDTH - 200, 10, 195, 20, ::std::string( LSTRING( "Units: ","Кол-во юнитов: " ) ) + IntToString( UnitsNum ) );

	if(Building)

//	DrawMyText(WIDTH-200,30,195,20,std::string("Производство: ")+IntToString((100*(GameTime%GROW_TIME))/GROW_TIME)+std::string("%"));
	DrawMyText_int_box(WIDTH-200,30,195,20,std::string(LSTRING("Production:","Производство: "))+IntToString(100-(100*Building)/GROW_TIME)+std::string("%"));
//else
...
void DrawMes()
{
	...
	static int ttt=100;
	static const ::std::string ps( LSTRING( "Pause", "Пауза" ) );

	if ( ( status & 16 ) != 0 ) {
		::glColor3d( 0.0f, 0.0f, 0.0f );
		DrawMyText_int_coords( WIDTH / 2 - 20, 10, 1, ps );
	}
...
//	::glBindTexture( GL_TEXTURE_2D, TEXTURE_WID.texID );
	...
	while(tmp)
	{
		...
		y-=MES_TXT_STEP;
		DrawMyText_int_coords(MES_BOX_X+10,y,0.8f,tmp->v.val);
		tmp=tmp->next;
	}
   ttt--;
...
//		glBindTexture( GL_TEXTURE_2D, TEXTURE_STATES.texID );
		::sf::Texture::bind( &( TEXTURE_STATES ) );
		for(i=0;i<3;i++)
		{
			x1=x2;
			x2+=ICON_SIZE;
...
//		glBindTexture ( GL_TEXTURE_2D, TEXTURE_FRAME(0).texID );
		::sf::Texture::bind( &( TEXTURE_FRAME( 0 ) ) );
		if(ct) DrawFrame(WIDTH-ICON_RAMA-ICON_SIZE*ct,int(y1),ICON_SIZE*ct,ICON_SIZE,-1);
...
void DrawMyText_float_coords(float x1,float y1,float scale, ::std::string const &s, MyFont& fnt)
{
...
//	glBindTexture( GL_TEXTURE_2D, TEXTURE_ARIAL_FONT.texID);
	::sf::Texture::bind( &( TEXTURE_ARIAL_FONT ) );
	for(i=0;i<n;i++)
	{
...
void DrawMyText_int_box(int x1,int y1,int w,int h, ::std::string const &s, MyFont& fnt)
{
	DrawMyText_float_box((float)x1,(float)y1,(float)w,(float)h,s,fnt);
}

void DrawMyText_float_box(float x1,float y1,float w,float h, ::std::string const &s, MyFont& fnt)
{
	...
	sc=h/(float)hh;
	DrawMyText_float_coords(x1+((w-ww*sc)*0.5f),y1+((h-hh*sc)*0.5f),sc,s,fnt);
}

void DrawMyText_int_coords( int x1, int y1, float scale, ::std::string const &s, MyFont& fnt) {
	DrawMyText_float_coords((float)x1,(float)y1,scale,s,fnt);
}
...
//	::glBindTexture( GL_TEXTURE_2D, TEXTURE_DIGITS.texID );
	::sf::Texture::bind( &( TEXTURE_DIGITS ) );
...
void DrawFrame(float x,float y,float w,float h,int txt) {
	if ( txt != -1 ) {
//		::glBindTexture( GL_TEXTURE_2D, TEXTURE_FRAME(txt).texID );
		::sf::Texture::bind( &( TEXTURE_DIGITS ) );
	}
...
//		::glBindTexture( GL_TEXTURE_2D, TEXTURE_WID.texID );
		...
		{
			glColor4fv(Shadow);
			DrawMyText_int_box(x,y+2,w,h-4,text);
			glColor3fv(White);
			DrawMyText_int_box(x-3,y,w+1,h-3,text);
		}else
		{
			glColor3fv(White);
			DrawMyText_int_box(x,y+2,w,h-4,text);
...
//		::glBindTexture( GL_TEXTURE_2D, TEXTURE_WID.texID );
		...
		DrawFrame(x,y,w,h,0);
		glColor3fv(TxtCol);
		DrawMyText_int_box(x,y+2,w,30,text);
		break;
	case 2:
		glColor3fv(TxtCol);
		DrawMyText_int_box(x,y,w,h,text);
		break;
...
//		glBindTexture ( GL_TEXTURE_2D, TEXTURE_WID.texID);
		...
		glColor3fv(White);
		DrawMyText_int_box(x,y,h,h,std::string("<"));
		DrawMyText_int_box(x+w-h,y,h,h,std::string(">"));
		DrawMyText_int_box(x,y+2,w,h-4,IntToString(text[0])+std::string("%"));
...
//					glBindTexture ( GL_TEXTURE_2D, TEXTURE_WID.texID);
				...
				DrawMyText_int_box(x,y1+2,w,h1-4,GetListString(text.c_str()+1,i));
			}
		}

		DrawMyText_int_box(x,y+2,w,h1-4,GetListString(text.c_str()+1,abs(text[0])-1));
